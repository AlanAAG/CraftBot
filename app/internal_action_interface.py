...
class InternalActionInterface:
    ...
    def perform_ocr(cls, image_path: str, user_prompt: Optional[str] = None) -> dict:
        ...
        Returns a status dict with the file path so the agent does not need to
        echo the full text back into its context window.
        """
        if cls.vlm_interface is None:
            raise RuntimeError("InternalActionInterface not initialized with VLMInterface.")
    ...
    def understand_video(
        ...
        Persists the summary to workspace to avoid flooding the agent context.
        """
        if cls.vlm_interface is None:
            raise RuntimeError("InternalActionInterface not initialized with VLMInterface.")
