...
from agent_core.external_libraries.recall.helpers.recall_helpers import (
    create_bot,
    get_bot,
    list_bots,
    delete_bot,
...
class RecallAppLibrary(ExternalAppLibrary):
    ...
    @classmethod
    def initialize(cls):
        """Initialize the Recall library with its credential store."""
        if cls._initialized:
            return
    ...
    @classmethod
    async def validate_connection_async(cls, user_id: str) -> bool:
        cred = await cls.get_credentials_async(user_id=user_id)
    ...
    @classmethod
    async def get_credentials_async(cls, user_id: str) -> Optional[RecallCredential]:
        cred_store = cls.get_credential_store()
        if cls._use_remote:
