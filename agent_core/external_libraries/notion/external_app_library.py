...
from agent_core.external_libraries.notion.helpers.notion_helpers import (
    search_notion,
    get_page,
    get_database,
    query_database,
...
class NotionAppLibrary(ExternalAppLibrary):
    ...
    @classmethod
    def initialize(cls):
        """Initialize the Notion library with its credential store."""
        if cls._initialized:
            return
    ...
    async def validate_connection_async(cls, user_id: str, workspace_id: Optional[str] = None) -> bool:
        cred = await cls.get_credentials_async(user_id=user_id, workspace_id=workspace_id)
