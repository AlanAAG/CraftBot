...
from agent_core.external_libraries.linkedin.helpers.linkedin_helpers import (
...
class LinkedInAppLibrary(ExternalAppLibrary):
    ...
    @classmethod
    def initialize(cls):
        """Initialize the LinkedIn library with its credential store."""
        if cls._initialized:
            return
    ...
    @classmethod
    async def validate_connection_async(cls, user_id: str, linkedin_id: Optional[str] = None) -> bool:
        cred = await cls.get_credentials_async(user_id=user_id, linkedin_id=linkedin_id)
    ...
    async def get_credentials_async(
        ...
        if cls._use_remote:
            if linkedin_id:
                credentials = await cred_store.get_async(user_id=user_id, linkedin_id=linkedin_id)
            else:
    ...
    def ensure_valid_token(
        ...
        In remote mode the backend auto-refreshes expired tokens,
        so we just fetch and return. In local mode we refresh here.
        """
        credential = cls.get_credentials(user_id=user_id, linkedin_id=linkedin_id)
        if not credential:
            return None

        # Remote mode: backend refreshes tokens before returning them
        if cls._use_remote:
            return credential
