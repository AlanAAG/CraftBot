...
class WhatsAppAppLibrary(ExternalAppLibrary):
    ...
    _session_restore_attempted: set = set()  # track session_ids we already tried to restore

    @classmethod
    def initialize(cls):
        """Initialize the WhatsApp library with its credential store."""
        if cls._initialized:
            return
    ...
    @classmethod
    async def validate_connection_async(cls, user_id: str, phone_number_id: Optional[str] = None) -> bool:
        cred = await cls.get_credentials_async(user_id=user_id, phone_number_id=phone_number_id)
        return cred is not None

    @classmethod
    def _ensure_session(cls, credential: WhatsAppCredential) -> None:
        """
        Lazy session restore: if no live Playwright session exists for
        this credential, try to reconnect it.

        Priority:
        1. Disk-based reconnect (local .whatsapp_web_sessions folder)
        2. session_data restore (from credential's storage_state)

        Only attempts once per session_id to avoid repeated failures.
        """
        session_id = credential.session_id or credential.phone_number_id
        if not session_id:
            return

        # Already tried this session — don't retry
        if session_id in cls._session_restore_attempted:
            return

        # Check if session is already live in the manager
        manager = get_whatsapp_web_manager()
        existing = manager.get_session(session_id)
        if existing and existing.status == "connected":
            return

        cls._session_restore_attempted.add(session_id)

        # Try 1: disk-based reconnect (local session folder)
        logger.info(f"[WhatsApp Web] No live session for {session_id}, attempting reconnect...")
        try:
            result = _run_async(reconnect_whatsapp_web_session(
                session_id=session_id,
                user_id=credential.user_id,
            ))
            if result.get("success") and result.get("status") == "connected":
                logger.info(f"[WhatsApp Web] Session {session_id} reconnected from disk")
                return
            logger.info(f"[WhatsApp Web] Disk reconnect result: {result.get('status', 'failed')} - {result.get('error', '')}")
        except Exception as e:
            logger.info(f"[WhatsApp Web] Disk reconnect not available: {e}")

        # Try 2: restore from credential's session_data
        session_data_str = credential.session_data
        if not session_data_str:
            logger.warning(
                f"[WhatsApp Web] No session_data in credential for {session_id} — "
                "cannot auto-restore. User needs to re-authenticate via QR code."
            )
            return
        ...
        try:
            ...
            logger.error(f"[WhatsApp Web] Invalid session_data for {session_id}")
            return

        logger.info(f"[WhatsApp Web] Restoring session {session_id} from credential session_data...")
        try:
            result = _run_async(restore_whatsapp_web_session(
                session_id=session_id,
                user_id=credential.user_id,
                session_data=session_data,
            ))
            if result.get("success"):
                logger.info(f"[WhatsApp Web] Session {session_id} restored: {result.get('status')}")
            else:
                logger.warning(f"[WhatsApp Web] Session restore failed: {result.get('error')}")
        except Exception as e:
            logger.error(f"[WhatsApp Web] Session restore error: {e}")
    ...
    def get_credentials(
        ...
        if credentials:
            ...
            # In remote mode, lazily restore the browser session if needed
            if cls._use_remote:
                cls._ensure_session(credential)
            return credential
        return None
    ...
    async def get_credentials_async(
        ...
        if cls._use_remote:
            if phone_number_id:
                credentials = await cred_store.get_async(user_id=user_id, phone_number_id=phone_number_id)
            else:
