...
from agent_core.external_libraries.google_workspace.helpers.google_helpers import (
    send_email_oauth2,
    encode_email,
...
from agent_core.external_libraries.google_workspace.helpers.google_drive_helpers import (
    list_drive_files,
    create_drive_folder,
    get_drive_file,
    move_drive_file,
...
class GoogleWorkspaceAppLibrary(ExternalAppLibrary):
    """Google Workspace integration library."""
    ...
    @classmethod
    def validate_connection(cls, user_id: str, email: str = None) -> bool:
        """Returns True if a credential exists for the given user_id and email."""
    ...
    def get_credentials(
        ...
        """Retrieve the credential for the given user_id and optional email."""
        cred_store = cls.get_credential_store()
        if email:
            credentials = cred_store.get(user_id=user_id, email=email)
        else:
    ...
    def send_email(
        ...
        """Send an email using the stored credential."""
        try:
    ...
    def resolve_drive_folder_path(
        ...
        """Resolve a Drive folder path into a folder ID."""
        credential = cls.ensure_valid_token(user_id=user_id, email=from_email)
        if not credential:
            return {"status": "error", "reason": "No valid credential found."}
