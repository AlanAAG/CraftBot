...
from agent_core.external_libraries.telegram.helpers.telegram_helpers import (
    get_me,
    send_message,
    send_photo,
    send_document,
...
try:
    from agent_core.external_libraries.telegram.helpers import telegram_mtproto_helpers as mtproto
...
class TelegramAppLibrary(ExternalAppLibrary):
    ...
    @classmethod
    def initialize(cls):
        """Initialize the Telegram library with its credential store."""
        if cls._initialized:
            return
    ...
    @classmethod
    async def validate_connection_async(cls, user_id: str, bot_id: Optional[str] = None) -> bool:
        cred = await cls.get_credentials_async(user_id=user_id, bot_id=bot_id)
    ...
    async def get_credentials_async(
        ...
        if cls._use_remote:
            if bot_id:
                credentials = await cred_store.get_async(user_id=user_id, bot_id=bot_id)
            else:
    ...
    def get_mtproto_credentials(
        ...
        """
        Retrieve MTProto (user account) credentials (sync - cache only in remote mode).
    ...
    @classmethod
    async def get_mtproto_credentials_async(
        cls,
        user_id: str,
        phone_number: Optional[str] = None,
    ) -> Optional[TelegramCredential]:
        """
        Retrieve MTProto (user account) credentials (async - fetches from backend).
        ...
        if cls._use_remote:
            credentials = await cred_store.get_async(user_id=user_id, **filters)
        else:
            credentials = cred_store.get(user_id=user_id, **filters)
        return credentials[0] if credentials else None
