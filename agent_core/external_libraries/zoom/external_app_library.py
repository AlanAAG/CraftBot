...
from agent_core.external_libraries.zoom.helpers.zoom_helpers import (
...
class ZoomAppLibrary(ExternalAppLibrary):
    ...
    @classmethod
    def initialize(cls):
        """Initialize the Zoom library with its credential store."""
        if cls._initialized:
            return
    ...
    @classmethod
    async def validate_connection_async(cls, user_id: str, zoom_user_id: Optional[str] = None) -> bool:
        cred = await cls.get_credentials_async(user_id=user_id, zoom_user_id=zoom_user_id)
    ...
    async def get_credentials_async(
        ...
        if cls._use_remote:
            if zoom_user_id:
                credentials = await cred_store.get_async(user_id=user_id, zoom_user_id=zoom_user_id)
            else:
