...
GUIモードは画面自動化を有効にします - エージェントがデスクトップ環境を見て操作できるようになります。
...
> [!NOTE]
> GUIモードは実験的機能であり、追加の依存関係が必要です（モデルウェイトで約4GB）。
...
| `--no-conda` | condaの代わりにグローバルpipを使用 |
...
| フラグ | 説明 |
|------|-------------|
| `--gui` | GUIモードを有効化（先に `install.py --gui` が必要） |
| `--no-conda` | condaの代わりにグローバルpipを使用 |
